...
**分类类型（kind）编辑时不可修改**：`kind` 是分类的语义身份，`summary.byCategory` 按 `categoryId` 归集交易、却按**交易**的 `kind` 过滤，从不校验 `category.kind`。把分类从「支出」改成「收入」后，历史支出交易仍挂在这个分类名下，支出统计里会混进收入分类名，且没有任何提示。真正的数据迁移要把该分类下所有历史交易的 `kind` 一并改掉，代价远大于「新建一个分类」。因此：**新增分类时可选类型，编辑既有分类时类型只读**（显示「支出（创建后不可修改）」+ 小字说明）。
