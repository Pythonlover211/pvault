...
- **先看大小再读**：超过 20MB 直接提示「请先按月拆分后再导入」，一次 `arrayBuffer()` 都不调
- 读之前先渲染「正在读取…」并让出一帧，再 `ArrayBuffer` → `decodeBytes` → `parseCsv`
- 两类文件级错误在第 1 步给出可执行的处置办法：未闭合的引号（`parseCsv` 抛错）、UTF-16 文件（`FF FE` / `FE FF` → 「请在 Excel 里另存为「CSV UTF-8(逗号分隔)」」）
...
- **收支方向留空时，方向的确定推到第 4 步**（必选的「这批金额全部算作」选择器），不在这里猜
- 预设命中时已自动填好（`autoMapping`）
- 「保存为格式配置」：一个名字输入框 + 保存按钮（预设命中时也可保存，方便用户改过之后复用）
- 已保存的配置显示为一个下拉，选中即套用

**第 4 步 · 预览与确认**
- 调 `prepareImport`，显示：**「将导入 N 条；跳过 M 条疑似重复；K 条无法解析；J 条不计收支，已跳过」**
- 无法解析的那些列出**前 5 条**（行号 + 原始值 + 原因）
- **收支方向列没有映射时**：一个**不预选**的「这批金额全部算作：支出 / 收入」选择器（角色的必修项 D1）——微信/支付宝的金额列永远是正数，漏配方向列会让整批 100% 变收入；未选时「确认导入」禁用，选定后作为 `defaultKind` 传给 `mapRows`
- 一个「默认支出分类」（下拉，支出分类）、一个「默认收入分类」（下拉，收入分类）与「默认账户」（下拉）选择器——导入的记录统一用它们，**`income` 记录绝不套支出分类**（否则首页工资显示成「🍜 餐饮 +12000」）；`transfer` 保持 `categoryId: null`
- 上面这组选择器要**紧跟摘要**，文案是诚实的：「导入的记录统一用下面这个分类和账户。当前版本不支持事后修改单条记录，选错只能撤销重导。」（角色的必修项 L2——统计页并没有改单条的入口，原来的「到统计页去改」是空头承诺）
- 一个「标题下显示前 20 条待导入记录」的预览表
- 底部「确认导入 N 条」——**只有 N > 0 且方向已明确时可用**
...
| 去重（时间 + 金额 + 收支方向 + 商户 指纹，提示「N 条重复已跳过」） | 2、4、5 |
| 先预览再落库（可勾选、可改分类、确认后才写入） | 5（V1 简化为「统一默认分类（支出/收入各一个）」，逐条改分类列入 V2） |
| V1 只支持 CSV，界面上写明 xlsx 请另存 | 5 |
| 收支方向必须明确（评审 D1 补入）：方向列未映射时必选、不预选、未选禁用确认 | 5 |
| 跳过的行必须计数（评审 L4 补入）：第四个数字「N 条不计收支，已跳过」 | 4、5 |

**与规格的一处有意简化**：规格 9 节写「解析后展示待导入列表，可勾选、可改分类」。V1 实现为**统一指定默认分类（支出/收入各一个）+ 默认账户**，不做逐条勾选与逐条改分类——理由是手机屏幕上逐条改几十上百条并不现实。评审指出原计划里「数据反正可以用统计页的分类功能事后调整」这句是错的（统计页没有编辑单条的入口），因此界面文案改为如实说明「当前版本不支持事后修改单条记录，选错只能撤销重导」（L2）。
